...
const PillarResults = ({ data, img }) => {
  ...
  return (
    ...
    {data.map((item, index) => (
        <motion.div variants={items} key={index} className={`bg-pmmGray overflow-hidden flex ${index !== 0 ? 'mt-0' : ''} rounded-lg border-0 border-gray-300`}>
            {/* Content of each result */}
            <p className="text-black-400 font-light p-1">{index + 1}. {item}</p>
        </motion.div>
    ))}
</motion.div>
